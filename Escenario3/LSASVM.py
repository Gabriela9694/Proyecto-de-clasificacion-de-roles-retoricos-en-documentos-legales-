...
from sklearn.svm import LinearSVC  # Usamos LinearSVC
...
def load_data(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            # Dividir la línea en doc_id y el resto
            parts = line.split(',', 1)
            if len(parts) < 2:  # Verificar que haya al menos una coma
                print(f"Formato incorrecto en línea: {line}")
                continue

            doc_id = parts[0]
            rest = parts[1].rsplit(',', 1)  # Dividir el resto en texto y etiqueta
            if len(rest) < 2:  # Verificar que haya al menos otra coma
                print(f"Formato incorrecto en línea: {line}")
                continue
...
# Si no hay datos de prueba, usar todo el conjunto como entrenamiento
if not has_test_data:
    print("\nNo se encontraron datos con etiqueta '???'. Usando todo el corpus como datos de entrenamiento.")
    train_data = data
    test_data = []  # No hay datos de prueba
...
# Pipeline combinado LSA + LDA con SVM
...
pipeline = Pipeline([
    ...
    ('clf', LinearSVC(class_weight='balanced', random_state=42))  # Usamos LinearSVC
])
...
params = {
    ...
    'clf__C': [0.1, 1.0, 10.0],  # Hiperparámetro de regularización
...
if has_test_data:
    ...
    with open('svm_classifications.txt', 'w') as f:
    ...
    print("\nArchivo 'svm_classifications.txt' generado con las predicciones.")
else:
